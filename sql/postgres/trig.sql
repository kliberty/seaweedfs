SELECT dump('filemeta');
DROP TABLE templ;
DROP TABLE filemeta_history CASCADE;

CREATE OR REPLACE FUNCTION dump(ag regclass)
    RETURNS void
    LANGUAGE plpgsql
AS
$$
DECLARE
    ...
    history_table TEXT;
BEGIN
    history_table = ag || '_history';
    EXECUTE format($t$CREATE TEMPORARY TABLE templ (LIKE %s)$t$, ag);
    ALTER TABLE templ
        ADD COLUMN sys_period tstzrange NOT NULL DEFAULT tstzrange(current_timestamp, null);
    EXECUTE format($t$ CREATE TABLE %s (LIKE templ) $t$, history_table);
    SELECT array_to_string(ARRAY(SELECT column_name
                                 FROM information_schema.COLUMNS
                                 WHERE table_name = ag::TEXT
	...
	$t$, history_table || '_trigger', history_table, history_table);
    EXECUTE format($t$ CREATE VIEW %s AS SELECT %s FROM %s $t$, history_table || '_view', cols, history_table);
    DROP TABLE templ;
    RETURN;
END
$$;
